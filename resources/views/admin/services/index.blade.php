...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
    <div>
        <h1 class="text-3xl font-bold tracking-tight text-zinc-900">Services</h1>
        <p class="text-zinc-500 text-sm mt-1">Kelola katalog perawatan kecantikan, tarif, dan ketersediaan layanan studio Anda.</p>
    </div>
    
    <a href="{{ route('admin.services.create') }}" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-zinc-900 text-zinc-50 px-4 py-2.5 rounded-md font-medium hover:bg-zinc-800 transition-colors shadow-sm text-sm tracking-wide">
        ...
        <table class="w-full border-collapse text-left">
            <thead>
                <tr class="bg-zinc-50/75 border-b border-zinc-200 text-zinc-500 font-medium text-xs uppercase tracking-wider">
                    ...
                    <th class="px-6 py-4">Harga (Tarif)</th>
                ...
                <tr class="hover:bg-zinc-50/50 transition-colors duration-200">
                    <td class="px-6 py-4">
                        <div class="flex items-center">
                            ...
                            <!-- Texts -->
                            <div class="ml-4">
                                <p class="font-semibold text-zinc-900 text-sm">{{ $service->name }}</p>
                                <p class="text-xs text-zinc-500 mt-0.5 max-w-md truncate">{{ $service->description ?: 'Tidak ada deskripsi layanan.' }}</p>
                            </div>
                        </div>
                    </td>
                    ...
                    <td class="px-6 py-4">
                        <div x-data="{ open: false }" class="relative inline-block text-left">
                            <button @click="open = !open" type="button" 
                                ...
                                {{ $service->status === 'active' ? 'bg-emerald-50 text-emerald-800 border-emerald-200 hover:bg-emerald-100/60' : '' }}
                                {{ $service->status === 'inactive' ? 'bg-zinc-100 text-zinc-800 border-zinc-200 hover:bg-zinc-150/60' : '' }}">
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-2.5">
                            <!-- Edit Button -->
                            <a href="{{ route('admin.services.edit', $service->id) }}" class="inline-flex items-center gap-1 border border-zinc-200 bg-white hover:bg-zinc-100 rounded-md text-xs font-medium text-zinc-700 px-3 py-1.5 transition-colors">
                ...
                <tr>
                    <td colspan="4" class="px-6 py-12 text-center">
                        <div class="max-w-md mx-auto py-6">
                            ...
                            <h3 class="font-semibold text-zinc-900 text-sm">Belum Ada Layanan</h3>
                            <p class="text-zinc-500 text-xs mt-1 max-w-xs mx-auto">Daftar katalog layanan kecantikan Studio Anda masih kosong. Silakan tambahkan layanan baru di atas.</p>
                        </div>
                    </td>
                </tr>
                @endforelse
...
<div id="delete-modal" class="fixed inset-0 z-50 flex items-center justify-center hidden">
    ...
    <div class="relative bg-white rounded-lg w-full max-w-[400px] p-6 shadow-lg border border-zinc-200 m-4 z-10 transform scale-95 opacity-0 transition-all duration-200" id="modal-box">
        <div class="space-y-4">
            <div class="space-y-2">
                <h3 class="text-lg font-semibold text-zinc-950 leading-none">Hapus Layanan?</h3>
                <p class="text-zinc-500 text-sm leading-normal">
                    Apakah Anda yakin ingin menghapus layanan <span id="target-service-name" class="font-medium text-zinc-900"></span>? Tindakan ini tidak dapat dibatalkan.
                </p>
            </div>
