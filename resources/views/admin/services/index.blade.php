...
@section('title', 'Daftar Layanan - Panel Admin Kwéla')

@section('content')
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10">
    <div>
        <p class="text-3xl md:text-4xl font-bold uppercase text-[#5d3a3a]">Daftar Layanan</p>
        <p class="text-stone-500 text-sm mt-0.5">Kelola katalog perawatan kecantikan, tarif, dan ketersediaan layanan studio Anda.</p>
    </div>
    
    <a href="{{ route('admin.services.create') }}" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-[#5d3a3a] text-white px-6 py-3.5 rounded-full font-semibold hover:bg-stone-900 transition-all duration-300 shadow-md hover:shadow-lg hover:scale-[1.02] active:scale-95 text-sm tracking-wide">
        ...
        <table class="w-full border-collapse text-left">
            <thead>
                <tr class="bg-stone-50/80 border-b border-stone-100">
                    <th class="px-8 py-5 text-xs font-bold text-stone-400 uppercase tracking-wider">Nama Layanan</th>
                    <th class="px-8 py-5 text-xs font-bold text-stone-400 uppercase tracking-wider">Investasi (Harga)</th>
                ...
                <tr class="hover:bg-stone-50/30 transition-colors duration-200 group">
                    <td class="px-8 py-5.5">
                        <div class="flex items-center">
                            ...
                            <div class="w-14 h-14 rounded-2xl overflow-hidden bg-stone-100 flex-shrink-0 border border-stone-200/40 relative group-hover:scale-105 transition-transform duration-300">
                                @if($service->image)
                                <img src="{{ asset('storage/' . $service->image) }}" alt="{{ $service->name }}" class="w-full h-full object-cover">
                                @else
                                <div class="w-full h-full bg-[#5d3a3a]/5 flex items-center justify-center text-xl">
                                    ✨
                                </div>
                                @endif
                            </div>
                            ...
                            <!-- Detail Teks -->
                            <div class="ml-5">
                                <p class="font-bold text-stone-900 group-hover:text-[#5d3a3a] transition-colors duration-200">{{ $service->name }}</p>
                                <p class="text-xs text-stone-400 mt-1 max-w-md truncate">{{ $service->description ?: 'Tidak ada deskripsi layanan.' }}</p>
                            </div>
                        </div>
                    </td>
                    
                    <td class="px-8 py-5.5">
                        <span class="font-semibold text-stone-800 text-sm bg-stone-100/70 border border-stone-200/30 px-3 py-1.5 rounded-xl">
                    ...
                    <td class="px-8 py-5.5 text-right">
                        <div class="flex items-center justify-end gap-3">
                            <!-- Edit Button -->
                            <a href="{{ route('admin.services.edit', $service->id) }}" class="inline-flex items-center gap-1.5 text-xs font-semibold text-stone-600 hover:text-stone-900 bg-stone-100/80 hover:bg-stone-200/60 px-3.5 py-2 rounded-xl transition-all duration-200">
                            ...
                            <button type="button" 
                                    onclick="triggerDeleteModal('{{ route('admin.services.destroy', $service->id) }}', '{{ $service->name }}')"
                ...
                <tr>
                    <td colspan="4" class="px-8 py-20 text-center">
                        <div class="max-w-md mx-auto">
                            ...
                            <h3 class="font-bold text-stone-800 text-lg">Belum Ada Layanan</h3>
                            <p class="text-stone-400 text-xs mt-1.5 max-w-sm mx-auto">Daftar katalog layanan kecantikan Studio Anda masih kosong. Silakan tambahkan layanan baru di atas.</p>
                        </div>
                    </td>
                </tr>
                @endforelse
...
<div id="delete-modal" class="fixed inset-0 z-50 flex items-center justify-center hidden">
    ...
    <div class="relative bg-white rounded-[2.5rem] w-full max-w-md p-8 shadow-2xl border border-stone-100 m-4 z-10 transform scale-95 opacity-0 transition-all duration-300" id="modal-box">
        <div class="text-center">
            ...
            <h3 class="text-xl font-bold text-stone-900 mb-2">Hapus Layanan?</h3>
            <p class="text-stone-500 text-sm leading-relaxed mb-6">
                Apakah Anda yakin ingin menghapus layanan <span id="target-service-name" class="font-bold text-stone-800"></span>? Tindakan ini tidak dapat dibatalkan.
            </p>
...
<script>
    function triggerDeleteModal(actionUrl, serviceName) {
        const modal = document.getElementById('delete-modal');
        const modalBox = document.getElementById('modal-box');
        const deleteForm = document.getElementById('delete-form');
        const targetNameSpan = document.getElementById('target-service-name');
        
        // Atur action form dan nama target
        deleteForm.action = actionUrl;
        targetNameSpan.textContent = serviceName;
