...
@section('title', 'Edit Promotion - Kwéla Beauty Admin')
...
<div class="mb-8 flex items-center justify-between">
    ...
    <a href="{{ route('admin.promotions.index') }}" class="inline-flex items-center gap-2 text-kwela-maroon hover:underline">
    ...
    <form action="{{ route('admin.promotions.update', $promotion->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mt-6">
            ...
            <textarea name="description" id="description" rows="4" 
                class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-kwela-maroon @error('description') border-red-500 @enderror">{{ old('description', $promotion->description) }}</textarea>
            @error('description')
            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mt-6">
            ...
            @if($promotion->image)
            <div class="mb-2">
                <img src="{{ asset('storage/' . $promotion->image) }}" alt="{{ $promotion->title }}" class="w-32 h-32 object-cover rounded">
            </div>
            @endif
        ...
        <div class="mt-8 flex justify-end space-x-4">
            <a href="{{ route('admin.promotions.index') }}" class="inline-flex items-center gap-2 px-6 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50">
