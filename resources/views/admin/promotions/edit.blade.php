...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
    <div>
        <h1 class="text-3xl font-bold tracking-tight text-zinc-900">Edit Promotion</h1>
        <p class="text-zinc-500 text-sm mt-1">Perbarui rincian program diskon, durasi aktif, dan materi visual promosi studio Anda.</p>
    </div>
...
<div class="bg-white rounded-lg border border-zinc-200 shadow-sm p-6 max-w-3xl">
    <form action="{{ route('admin.promotions.update', $promotion->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                <label for="discount" class="block text-xs font-semibold uppercase tracking-wider text-zinc-500 mb-1.5">Potongan Harga / Diskon (%) *</label>
                <div class="relative rounded-md shadow-sm">
                    <input type="number" name="discount" id="discount" value="{{ old('discount', $promotion->discount) }}" min="0" max="100"
                        class="w-full pl-3 pr-9 py-2 border border-zinc-200 rounded-md focus:outline-none focus:ring-2 focus:ring-zinc-950 focus:border-zinc-950 bg-white transition-all text-sm text-zinc-800 font-semibold @error('discount') border-red-500 @enderror"
                        required placeholder="0">
                    <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none text-zinc-400 font-bold text-sm">
                        %
                    </div>
                </div>
                @error('discount')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label for="start_date" class="block text-xs font-semibold uppercase tracking-wider text-zinc-500 mb-1.5">Tanggal Mulai Berlaku</label>
                <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $promotion->start_date?->format('Y-m-d')) }}" 
                    class="w-full px-3 py-2 border border-zinc-200 rounded-md focus:outline-none focus:ring-2 focus:ring-zinc-950 focus:border-zinc-950 bg-white transition-all text-sm text-zinc-800 font-semibold @error('start_date') border-red-500 @enderror">
                @error('start_date')
                <p class="mt-1.5 text-xs text-red-500 font-medium">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                <label for="end_date" class="block text-xs font-semibold uppercase tracking-wider text-zinc-500 mb-1.5">Tanggal Selesai / Kadaluwarsa</label>
                <input type="date" name="end_date" id="end_date" value="{{ old('end_date', $promotion->end_date?->format('Y-m-d')) }}" 
                    class="w-full px-3 py-2 border border-zinc-200 rounded-md focus:outline-none focus:ring-2 focus:ring-zinc-950 focus:border-zinc-950 bg-white transition-all text-sm text-zinc-800 font-semibold @error('end_date') border-red-500 @enderror">
                @error('end_date')
                <p class="mt-1.5 text-xs text-red-500 font-medium">{{ $message }}</p>
                @enderror
            </div>
        ...
        <!-- Row 4: Image -->
        <div>
            <span class="block text-xs font-semibold uppercase tracking-wider text-zinc-500 mb-2">Foto / Gambar Banner Promosi</span>
            <div class="grid grid-cols-1 sm:grid-cols-12 gap-6 items-center border border-zinc-200 p-4 rounded-md bg-zinc-50/50">
                ...
                <div class="sm:col-span-3 flex flex-col items-center justify-center border-r border-zinc-200 pr-0 sm:pr-6 py-1">
                    <span class="text-[10px] font-bold text-zinc-400 uppercase tracking-wider mb-2 text-center block">Banner Aktif</span>
                ...
                <!-- Upload new image -->
                <div class="{{ $promotion->image ? 'sm:col-span-9' : 'sm:col-span-12' }} w-full">
                    <label class="flex flex-col items-center justify-center w-full h-24 border border-zinc-200 border-dashed rounded-md cursor-pointer bg-white hover:bg-zinc-50/50 transition-colors group">
                        <div class="flex flex-col items-center justify-center text-center px-4">
                            ...
                            <p class="text-xs text-zinc-600 font-bold group-hover:text-zinc-800 transition-colors" id="file-upload-text">Klik untuk memilih banner baru</p>
        ...
        <div>
            ...
            <select name="status" id="status" 
                ...
                required>
                <option value="active" {{ $promotion->status == 'active' ? 'selected' : '' }}>Aktif (Tampil Online)</option>
                <option value="inactive" {{ $promotion->status == 'inactive' ? 'selected' : '' }}>Nonaktif (Ditangguhkan)</option>
            </select>
            @error('status')
            <p class="mt-1.5 text-xs text-red-500 font-medium">{{ $message }}</p>
            @enderror
...
<script>
    function updateUploadFeedback(input) {
        ...
        if (input.files && input.files.length > 0) {
            ...
            textElement.textContent = 'Klik untuk memilih banner baru';
